...
//		}
			...
			if(duplex==true) {
				...
				if(numOfPapers>amount) {
					numOfPapers=numOfPapers-amount;
					System.out.println("양면으로 "+amount+"장 출력했습니다. 현재"+numOfPapers+"장 남아있습니다.");
				}else {
					System.out.println("양면으로 모두 출력하려면 용지가 "+Math.abs(numOfPapers-amount)+"매 부족합니다. "+numOfPapers+"장만 출력합니다.");
					numOfPapers=0;
				}
			}else {
				if(numOfPapers>amount) {
